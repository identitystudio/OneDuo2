...
interface FrameAnalysis {
    ...
    // NEW: Visual description for PDF captions (fallback if no text)
...
Respond ONLY in this exact JSON format:
{
  ...
  "visualDescription": "A clear, 1-2 sentence description of what is happening visually for a caption",
  "textType": "slide|document|ui|code|other",
  "emphasisFlags": {
    "highlight_detected": boolean,
    "cursor_pause": boolean,
  ...
  "dependsOnPrevious": boolean
        ...
        for (let i = 0; i < finalFrameUrls.length; i += effectiveBatchSize) {
            ...
            const batchPromises = batch.map(async (frameUrl: string, batchIndex: number) => {
                ...
                // Fetch and encode image for Gemini
                let base64Image = '';
                ...
                try {
                    ...
                    console.error(`[extract-frame-text] Failed to fetch/encode image for frame ${frameIndex}:`, imgError);
                    return null;
                }
                ...
                for (let attempt = 0; attempt <= maxRetries; attempt++) {
                    try {
                        ...
                        if (!response.ok) {
                            ...
                            console.error(`[extract-frame-text] Failed to analyze frame ${frameIndex}: ${response.status}`, errBody);
                        ...
                        try {
                            ...
                            return {
                                ...
                                timestamp,
                                text: parsed.text || '',
                                visualDescription: parsed.visualDescription || parsed.instructorIntent || '', // Fallback
